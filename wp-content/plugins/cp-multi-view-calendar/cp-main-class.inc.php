...
<script type='text/javascript'>
/* <![CDATA[ */ 
var cpmvc_configmultiview<?php echo ($this->print_counter); ?> = {"obj":"{\"params\":<?php echo str_replace('"','\"',str_replace('\\','\\\\',$params)); ?>,\n  \"ajax_url\":\"<?php echo str_replace(array('"', "'"),array('\\"', "\\'"),$this->get_site_url()); ?>/?cpmvc_do_action=mvparse&v=<?php echo $atts["view"]; ?>\",\n               \"calendar\":\"<?php echo str_replace(array('"', "'"),array('\\"', "\\'"),$base_params["id"]); ?>\"\n  \t  }"};
/* ]]> */
</script>
<script type='text/javascript' src='<?php echo plugins_url('/DC_MultiViewCal/src/Plugins/multiview.public.js', __FILE__); ?>'></script>
<?php
...
<?php
    ...
    function save_edition()
    {
        ...
        if ($_POST["cfwpp_edit"] == 'js')   
            update_option('CP_MCV_JS', base64_encode($_POST["editionarea"]));  
        else if ($_POST["cfwpp_edit"] == 'css')  
            update_option('CP_MCV_CSS', base64_encode($_POST["editionarea"]));  
    }
    
    function data_management() {
        global $wpdb;
        
        if ( 'POST' == $_SERVER['REQUEST_METHOD'] && isset( $_POST['CP_MCV_post_edition'] ) && is_admin() )
        {
            $this->save_edition();
            return;
        }  
